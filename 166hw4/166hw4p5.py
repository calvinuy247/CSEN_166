...
for k in range(max_iteration):  
    ...
    # no spin:
    high_nospin = T[0,1,1]*(R[0,1,1]+gamma*V[1,k]) + T[0,0,1]*(R[0,0,1]+gamma*V[0,k])
    V[0,k+1]=np.max([high_spin,high_nospin])
    
    # medium:
    # spin:
    medium_spin = T[1,0,0]*(R[1,0,0]+gamma*V[0,k]) + T[1,1,0]*(R[1,1,0]+gamma*V[1,k])
    # no spin:
    medium_nospin = T[1,2,1]*(R[1,2,1]+gamma*V[2,k]) + T[1,1,1]*(R[1,1,1]+gamma*V[1,k])
    V[1,k+1]=np.max([medium_spin,medium_nospin])
    
    # low
    # spin:
    low_spin = T[2,1,0]*(R[2,1,0]+gamma*V[1,k]) + T[2,2,0]*(R[2,2,0]+gamma*V[2,k])
    # no spin:
